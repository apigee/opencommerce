...
token=`curl -X POST https://${UGURI}/management/token  -d '{"grant_type":"client_credentials", "client_id": "'${UGCLIENTID}'", "client_secret": "'${UGCLIENTSECRET}'"}' | sed 's/.*access_token\"\:\"\(.*\)\"\,\"expires_in.*/\1/'`

echo "Create App Services Application: ${UGAPP}, with Token: ${token}"
curl -X POST https://${UGURI}/management/orgs/${UGORG}/apps?access_token=${token} -d '{"name":"'${UGAPP}'"}'

creds=`curl -X POST https://${UGURI}/management/orgs/${UGORG}/apps/${UGAPP}/credentials?access_token="${token}" | sed 's/}//'`
